...
class UpscaleJobs:
    ...
    def public(self,job):
        directory=self.path/job['id'];available=False
        try:available=(directory/'result.png').is_file() and json.loads((directory/'status.json').read_text('utf-8')).get('state')=='completed'
    ...
    def submit(self,body):
        ...
        with self.lock:
            ...
            directory=self.path/ident;directory.mkdir()
        ...
        with self.lock:
            for job in self.jobs():
                if job['status'] in ACTIVE|{'queued'}:
                    job.update(status='interrupted',message='アプリ終了により中断しました。自動で再推論しません。',ended=now())
                    self.save(job)
    ...
    def run(self,ident):
        with self.lock:
            ...
            job.update(status='loading',started=now(),started_epoch=time.time(),message='専用子プロセスを起動中')
        ...
        try:
            ...
            self.process=subprocess.Popen([job['machine']['python'],'-I',str(self.runner),str(directory/'request.json')],
    ...
    def stop_child(self,directory):
        ...
        try:self.process.wait(timeout=5)
            ...
            if os.name=='nt':
                # The venv launcher can have a Python child. Stop only our process
                # tree, rather than leaving a CUDA child behind on launcher exit.
    ...
    def finalize(self,ident):
        with self.lock:
            ...
            if status.get('state')!='completed' or not (directory/'result.png').is_file():raise ValueError('再利用できる推論結果がありません。')
            job.update(status='saving',environment=status['environment'],geometry=status.get('geometry'),native_scale_verified=not status['environment'].get('mock',False))
            self.save(job)
            try:
