...
import {enhancementState,enhancementVisible} from './enhancementState';
import {api} from './api';
import {Registration,readRecord} from './workspace';

export default function PromptEnhancer({prompt,context,resultId,onResult,appliedId,draftId,flushDraft,onApply,onRestore}){
 ...
 const ident=appliedId||resultId||pending?.jobId||pending?.payload?.id;
 useEffect(()=>{
 ...
 const {active,canApply,previous}=enhancementState(job,prompt,matching,appliedId);
 const showResult=enhancementVisible(job,prompt,matching,appliedId,resultId);
 ...
 async function run(reconcile=false){
  ...
  try{
   if(reconcile){const next=await manager.current.reconcile();setJob(next);if(next)onResult(next.id);}
   else{await flushDraft();const next=await manager.current.begin({...context,model:'Qwen-Image-2.1',prompt,max_new_tokens:limit,seed:42});setJob(next);onResult(next.id);}
 ...
 return <section className="prompt-enhancer"><strong>Qwenで指示を補強（{i2i?'PE-I2I':'PE-T2I'}）</strong><p>{i2i?'生成に使う元画像・参照画像・マスクと指示文を読み取ります。':'参照画像なしの新規生成用です。'}補強後の英語文を確認して採用できます。補強だけでは画像を生成せず、指定寸法も変更しません。</p>
  ...
  {job&&(working||unresolved||showResult)?<p role="status">{previous?'以前の補強結果です。現在の指示には適用されていません。':active?'この補強結果を採用中です。':job.message}{job.worker_error?' / '+job.worker_error:''}</p>:null}{error?<p role="status" className="warning">{error}</p>:null}
  {showResult?<><details><summary>補強前の指示</summary><pre>{job.params.prompt}</pre></details><label className="field">{previous?'以前の補強結果（英語）':'補強結果（英語）'}<textarea readOnly value={job.result.rewritten_prompt}/></label><p>推奨縦横比：{job.result.wh_ratio||job.result.ratio_follow+'に合わせる'}（現在の幅・高さはそのまま）</p>
   {canApply?<button onClick={()=>onApply(job)}>この補強結果を採用</button>:null}{active?<button onClick={()=>onRestore(job.params.prompt)}>補強前に戻す</button>:null}{previous?<small>現在の指示を補強する場合は「指示を補強する」を押してください。</small>:null}
  </>:null}
