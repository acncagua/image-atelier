...
export default function PromptEnhancer({prompt,draftId,flushDraft,onApply,onRestore}){
 ...
 async function run(reconcile=false){
  ...
  try{
   ...
   else{await flushDraft();setJob(await manager.current.begin({model:'Qwen-Image-2.1',mode:'generate',refs:[],prompt,max_new_tokens:limit,seed:42}));}
  }catch(e){setError(e.message);}finally{setPending(manager.current.record);gate.current=false;setBusy(false);}
 }
 return <section className="prompt-enhancer"><strong>Qwenで指示を補強（PE-T2I）</strong><p>参照画像なしの新規生成用です。補強後の英語文を確認して採用できます。補強だけでは画像を生成せず、指定寸法も変更しません。</p>
  ...
  {job?.status==='completed'&&job.result?<><details><summary>補強前の指示</summary><pre>{job.params.prompt}</pre></details><label className="field">補強結果（英語）<textarea readOnly value={job.result.rewritten_prompt}/></label><p>推奨縦横比：{job.result.wh_ratio}（現在の幅・高さはそのまま）</p>
   <button disabled={prompt!==job.params.prompt} onClick={()=>onApply(job)}>この補強結果を採用</button>{prompt!==job.params.prompt?<button onClick={()=>onRestore(job.params.prompt)}>補強前に戻す</button>:null}{prompt!==job.params.prompt?<small>採用済み、または元の指示が変更されています。必要なら現在の指示で再補強してください。</small>:null}
  </>:null}
  <details><summary>補強モデルの設定</summary><label className="field">出力トークン上限<input aria-label="補強トークン上限" type="number" min="256" max="16256" value={limit} onChange={e=>setLimit(Number(e.target.value))}/></label>
   {config?<><label className="field">PE専用Python<input value={config.python} onChange={e=>setConfig({...config,python:e.target.value})}/></label><label className="field">PE-T2Iモデルフォルダー<input value={config.model} onChange={e=>setConfig({...config,model:e.target.value})}/></label><button disabled={busy||working} onClick={async()=>{try{await api('pe/config',config);setError('補強モデルの設定を保存しました。');}catch(e){setError(e.message);}}}>補強設定を保存</button></>:null}
  </details>
 </section>;
}
