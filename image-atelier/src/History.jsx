...
export default function History({jobs,onLoad,onResult,onEdit,onCancel,onReprocess}){
 ...
 <button onClick={()=>onLoad(j)}>条件を復元・新しく実行の準備</button>{j.status==='queued'?<button onClick={()=>onCancel(j.id)}>待機取消</button>:null}{j.recovery&&j.status!=='sending'?<button onClick={()=>onReprocess(j.id)}>保存応答を再処理（無料・API再送なし）</button>:null}
