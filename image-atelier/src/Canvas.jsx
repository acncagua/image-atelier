...
export default function Canvas({asset,strokes=[],setStrokes,tool='pan',brush=40,zoom,setZoom,pan,setPan,onCrop,onOpen,label}){
 ...
 return <section className="image-panel"><h2>{label}<span>{asset?`${asset.width} × ${asset.height}`:''}</span></h2><div className={'viewport tool-'+tool} ref={host} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={()=>{drag.current=null;}} onWheel={e=>{if(asset){setZoom(Math.max(.05,Math.min(8,scale*(e.deltaY<0?1.1:.9))));}}}>
