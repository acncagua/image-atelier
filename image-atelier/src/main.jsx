...
function App(){
 ...
 function loadParameters(next,saved){setP(next);setInstructionEditors({change:restoreInstructions('change',next.change||'',saved?.change),keep:restoreInstructions('keep',next.keep||'',saved?.keep)});}
 ...
 async function execute(){
  ...
  if(p.provider==='openai'&&retrySource&&!window.confirm('結果不明の履歴から新しいジョブを実行します。前回分と今回分の両方が課金される可能性があります。実行しますか？'))return;
 ...
 {p.mode!=='upscale'&&p.width*p.height>2560*1440?<p className="warning">2560×1440を超える解像度は実験的です。</p>:null}
