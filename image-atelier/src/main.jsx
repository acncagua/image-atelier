...
function App(){
 ...
 async function execute(){
  ...
  if(pending?.state==='unconfirmed'){setMessage('未確認の登録を先に確認してください。確定済みのIDと入力を保持しています。');return;}
 ...
 if(!boot||!draft.ready)return <main className="loading">{message||draft.status||'Image Atelier を起動しています…'}<p>{health?.message}</p><button onClick={()=>window.location.reload()}>再読み込み</button></main>;
 ...
 {pending?.state==='unconfirmed'?<div className="notice"><span>未確認の登録: {pending.payload.id}。送信時の入力を固定して保持しています。</span><button disabled={busy} onClick={reconcile}>登録状況を確認（同じID）</button></div>:null}
