...
import {composeInstructions,restoreInstructions,parseInstructions} from './instructionPresets';
import './styles.css';
import useDraft from './useDraft';
import {Registration,readRecord,listRecords,writeRecord,archiveDraft} from './workspace';
import UpscaleControls,{upscaleDefaults} from './Upscale';
import {restoreUpscaleOptions} from './upscalePlan';
...
function App(){
 ...
 const [instructionEditors,setInstructionEditors]=useState(()=>({change:parseInstructions('change',defaults.change),keep:parseInstructions('keep',defaults.keep)}));
 function loadParameters(next,saved){setP({...next,provider:next.provider==='qwen'?'mock':next.provider});setInstructionEditors({change:restoreInstructions('change',next.change||'',saved?.change),keep:restoreInstructions('keep',next.keep||'',saved?.keep)});}
 function updateInstructions(kind,editor){setInstructionEditors(old=>({...old,[kind]:editor}));setP(old=>({...old,[kind]:composeInstructions(kind,editor)}));}
 ...
 const [upscaleSource,setUpscaleSource]=useState('target'),[upscaleOptions,setUpscaleOptions]=useState(upscaleDefaults),[gpuJobs,setGpuJobs]=useState([]),[upscaleReady,setUpscaleReady]=useState(false);
 ...
 const isQwen=p.model===QWEN_MODEL;
 ...
 async function restoreDraft(saved,warnings=[]){
  ...
  setUpscaleSource(saved.upscaleSource==='result'?'result':'target');setUpscaleOptions(restoreUpscaleOptions(saved.upscaleOptions));
 ...
 useEffect(()=>{Promise.all([bootstrap(),api('jobs'),api('presets'),api('local-edits')]).then(([b,j,pr,l])=>{setBoot(b);setSettings(b.settings);setJobs(j);setPresets(pr);setLocalEdits(l);}).catch(e=>setMessage(e.message));let mounted=true;const timer=setInterval(()=>{
  api('upscale/jobs').then(j=>{if(mounted)setGpuJobs(j);}).catch(()=>{});
 ...
 async function adjustAsset(asset,input,options={width:p.width,height:p.height,method:adjust}){await act(async()=>{
  ...
  const adjusted=await api('resize',{id:asset.id,...options,context:{...params(),mode:p.mode==='upscale'?'polish':p.mode}});
 ...
 async function refreshGPU(){setGpuJobs(await api('upscale/jobs'));}
 async function restoreUpscale(job){await act(async()=>{const image=await api('assets/'+job.params.source_id);setFollow(null);setBase(image);setResult(job.output||null);loadParameters({...defaults,...job.params.context,mode:'upscale'});setRefs(job.params.context?.refs||[]);setUpscaleOptions(restoreUpscaleOptions(job.params.options));setUpscaleSource('target');setTool('pan');window.scrollTo({top:0,behavior:'instant'});});}
 ...
 function editHistory(job,asset){
  ...
  loadParameters({...next,mode:'polish',width:asset.width,height:asset.height,n:1,composite:false});
 ...
 async function execute(){
  ...
  if(qwenProblem(p,target)){setMessage(qwenProblem(p,target));return;}
  ...
  if(!isQwen&&p.provider==='openai'&&retrySource&&!window.confirm('結果不明の履歴から新しいジョブを実行します。前回分と今回分の両方が課金される可能性があります。実行しますか？'))return;
 ...
 return <><header><h1>Image Atelier</h1><div><span className="connection">{p.mode==='upscale'?'ローカルGPU':isQwen?'QwenローカルGPU':p.provider==='mock'?'モック接続':boot.key_set?'APIキー設定済み・アクセス未検証':'APIキー未設定'}</span><button onClick={()=>act(async()=>{const r=await api('output-folder/open',{});setMessage('画像フォルダーを開きました: '+r.path);})}>画像フォルダーを開く</button><button onClick={()=>setSettingsOpen(!settingsOpen)}>設定</button></div></header>
 ...
 {target&&(target.width%(isQwen?32:16)!==0||target.height%(isQwen?32:16)!==0)?<div className="input-alignment"><button onClick={()=>adjustAsset(target,true,{width:Math.ceil(target.width/(isQwen?32:16))*(isQwen?32:16),height:Math.ceil(target.height/(isQwen?32:16))*(isQwen?32:16),method:'pad'})}>{isQwen?32:16}の倍数に調整</button><small>{target.width}×{target.height} → {Math.ceil(target.width/(isQwen?32:16))*(isQwen?32:16)}×{Math.ceil(target.height/(isQwen?32:16))*(isQwen?32:16)}（透明余白を追加）</small></div>:null}
 {target&&p.mode!=='upscale'?<details><summary>編集対象の寸法を調整</summary><p>現在 {target.width}×{target.height} → 上部の指定 {p.width}×{p.height}</p>{inputSizeWarning&&!isQwen?<p className="warning">入力寸法はAPI条件外です。16pxの倍数などの条件に合わせ、方法を選んで調整してください。候補: {Math.ceil(target.width/(isQwen?32:16))*(isQwen?32:16)}×{Math.ceil(target.height/(isQwen?32:16))*(isQwen?32:16)}（余白）または1920×1088。自動変形はしません。</p>:null}<select aria-label="入力の寸法調整方法" value={adjust} onChange={e=>setAdjust(e.target.value)}><option value="crop">中央を切り抜き</option><option value="pad">透明余白を追加</option><option value="resize">リサイズ（変形を含む）</option></select><button onClick={()=>adjustAsset(target,true)}>入力を別画像として調整</button></details>:null}
 <div hidden={p.mode==='upscale'}><DropZone title="参照資料" multiple onFiles={fs=>files(fs,true)}/><References refs={refs} setRefs={setRefs}/><small>参照資料 {refs.length} / {referenceLimit(p)}枚{isQwen?'（元画像・マスクを含め全10枚）':''}</small>{refs.length>referenceLimit(p)?<p className="warning">参照資料が上限を超えています。資料を減らしてください。</p>:null}
 <InstructionEditor kind="change" value={instructionEditors.change} onChange={editor=>updateInstructions('change',editor)}/>
 <InstructionEditor kind="keep" value={instructionEditors.keep} onChange={editor=>updateInstructions('keep',editor)}/>
 <details><summary>キャラクター資料と維持指示の保存</summary><input aria-label="プリセット名" placeholder="キャラクター名など" value={presetName} onChange={e=>setPresetName(e.target.value)}/><button onClick={()=>act(async()=>setPresets(await api('presets',{name:presetName,keep:p.keep,refs}))) }>資料と維持指示を保存</button>{presets.map(pr=><button key={pr.name} onClick={()=>{updateInstructions('keep',parseInstructions('keep',pr.keep||''));setRefs(pr.refs);}}>{pr.name}</button>)}</details>
 </div><div className="run-actions">{isQwen?<p>QwenローカルGPU・API料金なし</p>:<UsagePanel usage={usage} error={usageError} jobs={jobs} p={p}/>}{p.mode!=='upscale'&&promptArchive.length?<details><summary>以前の送信指示文（回収用）</summary>{promptArchive.map((text,i)=><pre key={i}>{text}</pre>)}</details>:null}<button hidden={p.mode==='upscale'} className="wide" onClick={preview}>指示文を確認・編集</button>{p.mode==='upscale'?<button className="primary wide" type="submit" form="upscale-execution" disabled={!upscaleReady}>アップスケール実行</button>:<button className="primary wide" disabled={busy||!registrationReady||!!qwenProblem(p,target)||refs.length>referenceLimit(p)||['unconfirmed','unsaved'].includes(pending?.state)} onClick={execute}>{busy?'登録中…':isQwen?'Qwenで実行（ローカル）':p.provider==='mock'?'モックで実行':'実APIで生成（有料）'}</button>}</div>
 <small hidden={p.mode==='upscale'}>資料の役割は指示文による指定です。顔の同一性や塗りの一致を保証するものではありません。</small>
 </aside><main>
 <div className="toolbar"><select aria-label="モード" value={p.mode} onChange={e=>{setP(old=>({...old,mode:e.target.value,...(isQwen&&e.target.value==='inpaint'?{composite:true}:{})}));setTool('pan');}}><option value="polish">ブラッシュアップ</option><option value="generate">新規生成</option><option value="inpaint">部分修正</option><option value="upscale">アップスケール</option></select>{p.mode==='upscale'?<select aria-label="アップスケーラー" defaultValue="swinir"><option value="swinir">SwinIR</option></select>:<><select aria-label="モデル" value={p.model} onChange={e=>setP(old=>({...old,model:e.target.value,...(e.target.value===QWEN_MODEL?{format:'png',n:1,...(old.mode==='inpaint'?{composite:true}:{})}:{})}))}>{Object.keys(boot.capabilities.models).map(m=><option key={m}>{m}</option>)}<option value={QWEN_MODEL}>{QWEN_MODEL}（ローカル）</option></select><select hidden={isQwen} aria-label="品質" value={p.quality} onChange={e=>change('quality',e.target.value)}>{(boot.capabilities.models[p.model]?.qualities||[]).map(q=><option key={q}>{q}</option>)}</select><div className="dimensions"><input aria-label="幅" type="number" step={isQwen?32:16} value={p.width} onChange={e=>dimension('width',e.target.value)}/><span>×</span><input aria-label="高さ" type="number" step={isQwen?32:16} value={p.height} onChange={e=>dimension('height',e.target.value)}/></div><select disabled={isQwen} aria-label="出力形式" value={p.format} onChange={e=>change('format',e.target.value)}>{boot.capabilities.formats.map(f=><option key={f} value={f}>{f.toUpperCase()}</option>)}</select><select disabled={isQwen} aria-label="生成枚数" value={p.n} onChange={e=>change('n',Number(e.target.value))}>{[1,2,3,4].map(n=><option key={n} value={n}>{n}枚</option>)}</select></>}</div>
 {isQwen&&p.mode!=='upscale'?<QwenControls p={p} change={change} target={target}/>:null}
 <div className="subtoolbar" hidden={p.mode==='upscale'}><button onClick={()=>setP({...p,width:p.height,height:p.width})}>縦横交換</button><label><input type="checkbox" checked={lockRatio} onChange={e=>setLockRatio(e.target.checked)}/>比率保持（{isQwen?32:16}px単位）</label><select aria-label="寸法プリセット" defaultValue="" onChange={e=>{if(e.target.value){const [width,height]=e.target.value.split('x').map(Number);setP({...p,width,height});e.target.value='';}}}><option value="">寸法プリセット</option><option value="1920x1088">1920×1088（SD）</option><option value="2048x1152">2048×1152（16:9）</option><option value="1024x1024">1024×1024</option></select><span>比較はズーム・移動が連動</span></div>
 {!isQwen&&p.mode!=='upscale'&&p.width*p.height>2560*1440?<p className="warning">2560×1440を超える解像度は実験的です。</p>:null}
 {p.mode==='inpaint'?<div className="mask-toolbar">{[['pan','移動'],['brush','ブラシ'],['erase','消しゴム']].map(([v,l])=><button className={tool===v?'selected':''} key={v} onClick={()=>setTool(v)}>{l}</button>)}<label>ブラシ径 <input aria-label="ブラシ径" type="range" min="2" max="300" value={brush} onChange={e=>setBrush(e.target.value)}/>{brush}px</label><button disabled={!strokes.length} onClick={()=>{setRedo([...redo,strokes.at(-1)]);setStrokes(strokes.slice(0,-1));}}>取り消し</button><button disabled={!redo.length} onClick={()=>{setStrokes([...strokes,redo.at(-1)]);setRedo(redo.slice(0,-1));}}>やり直し</button><button onClick={()=>{setStrokes([]);setRedo([]);}}>全消去</button><label><input type="checkbox" checked={p.composite} onChange={e=>change('composite',e.target.checked)}/>変更範囲だけ合成</label><label>境界ぼかし <input aria-label="境界ぼかし" type="number" min="0" max="100" value={p.feather} onChange={e=>change('feather',Number(e.target.value))}/>px</label><small>塗った場所＝変更範囲。モデルへのマスク指定だけでは厳密な画素保護になりません。局所合成は境界を内側にぼかし、塗っていない画素を保持します。</small></div>:null}
 <div hidden={p.mode!=='upscale'}><UpscaleControls asset={upscaleSource==='target'?target:result} onAvailability={setUpscaleReady} source={upscaleSource} setSource={setUpscaleSource} context={{...params(),mode:'polish'}} options={upscaleOptions} setOptions={setUpscaleOptions} draftId={draft.id} flushDraft={draft.flush} jobs={gpuJobs} onRefresh={refreshGPU} onRegistered={job=>{if(job?.params?.kind!=='diagnostic')setFollow(job.id);}}/></div>
 <div className="compare"><Canvas label="元画像" asset={target} strokes={p.mode==='inpaint'?strokes:[]} setStrokes={updater=>{setStrokes(updater);setRedo([]);}} tool={p.mode==='inpaint'||tool==='crop'?tool:'pan'} brush={brush} zoom={zoom} setZoom={setZoom} pan={pan} setPan={setPan} onCrop={crop}/><Canvas label="結果" asset={result} onOpen={()=>{setFollow(null);setViewerAsset(result);}} zoom={zoom} setZoom={setZoom} pan={pan} setPan={setPan}/></div>
 {result?<div className="result-actions"><button onClick={()=>setViewerAsset(result)}>大きく表示</button><strong>{result.kind==='composite'?'局所合成':result.name}</strong><button onClick={()=>{setFollow(null);setBase(result);setTool('pan');setP(old=>({...old,mode:'polish',width:result.width,height:result.height}));setMessage('結果を次の編集対象にしました。資料と指示を引き継ぎます。');}}>次の編集対象にする</button><button onClick={()=>act(async()=>{const r=await api('export',{id:result.id});setMessage('保存しました: '+r.path);})}>設定フォルダーにPNG保存</button><a href={'/api/assets/'+result.id+'/download'} download title="ブラウザーのダウンロード先へPNGを保存">PNGダウンロード</a><details hidden={p.mode==='upscale'}><summary>寸法を明示的に調整</summary><p>上部の幅・高さに合わせて別画像を作成します。</p><select aria-label="寸法調整方法" value={adjust} onChange={e=>setAdjust(e.target.value)}><option value="crop">中央を切り抜き</option><option value="pad">透明余白を追加</option><option value="resize">リサイズ（変形を含む）</option></select><button onClick={()=>adjustAsset(result,false)}>別画像として作成</button></details></div>:null}
 <HistoryTabs mode={p.mode} localEdits={localEdits} onLocalEdit={entry=>editHistory({params:entry.context||defaults},entry.result)} onGPULoad={restoreUpscale} jobs={jobs} gpuJobs={gpuJobs} onGPURefresh={refreshGPU} onLoad={restore} onEdit={editHistory} onResult={asset=>{setFollow(null);setResult(asset);setViewerAsset(asset);}} onReprocess={id=>act(async()=>{await api('jobs/'+id+'/reprocess',{});await refresh();setMessage('保存済み結果を再処理しました。再生成はしていません。');})} onCancel={id=>act(async()=>{await api('jobs/'+id+'/cancel',{});await refresh();})}/>
