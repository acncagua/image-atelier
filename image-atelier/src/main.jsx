...
function App(){
 ...
 async function restoreUpscale(job){await act(async()=>{const image=await api('assets/'+job.params.source_id);setFollow(null);setBase(image);setResult(job.output||null);setP({...defaults,...job.params.context,mode:'upscale'});setRefs(job.params.context?.refs||[]);setUpscaleOptions(restoreUpscaleOptions(job.params.options));setUpscaleSource('target');setTool('pan');window.scrollTo({top:0,behavior:'instant'});});}
 ...
 return <><header><h1>Image Atelier</h1><div><span className="connection">{p.mode==='upscale'?'ローカルGPU':p.provider==='mock'?'モック接続':boot.key_set?'APIキー設定済み・アクセス未検証':'APIキー未設定'}</span><button onClick={()=>setSettingsOpen(!settingsOpen)}>設定</button></div></header>
