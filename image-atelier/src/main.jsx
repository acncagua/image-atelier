...
import {QWEN_MODEL,qwenDefaults,qwenProblem} from './qwenOptions';
...
function App(){
 ...
 async function execute(){
  ...
  if(qwenProblem(p)){setMessage(qwenProblem(p));return;}
 ...
 {target&&(target.width%16!==0||target.height%16!==0)?<div className="input-alignment"><button onClick={()=>adjustAsset(target,true,{width:Math.ceil(target.width/16)*16,height:Math.ceil(target.height/16)*16,method:'pad'})}>16の倍数に調整</button><small>{target.width}×{target.height} → {Math.ceil(target.width/16)*16}×{Math.ceil(target.height/16)*16}（透明余白を追加）</small></div>:null}
 {target&&p.mode!=='upscale'?<details><summary>編集対象の寸法を調整</summary><p>現在 {target.width}×{target.height} → 上部の指定 {p.width}×{p.height}</p>{inputSizeWarning&&!isQwen?<p className="warning">入力寸法はAPI条件外です。16pxの倍数などの条件に合わせ、方法を選んで調整してください。候補: {Math.ceil(target.width/16)*16}×{Math.ceil(target.height/16)*16}（余白）または1920×1088。自動変形はしません。</p>:null}<select aria-label="入力の寸法調整方法" value={adjust} onChange={e=>setAdjust(e.target.value)}><option value="crop">中央を切り抜き</option><option value="pad">透明余白を追加</option><option value="resize">リサイズ（変形を含む）</option></select><button onClick={()=>adjustAsset(target,true)}>入力を別画像として調整</button></details>:null}
 <div hidden={p.mode==='upscale'}><DropZone title="参照資料" multiple onFiles={fs=>files(fs,true)}/><References refs={refs} setRefs={setRefs}/>
