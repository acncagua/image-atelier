...
`Dev-qwen`ブランチでは、既存のモデル選択に「Qwen-Image-2.1（ローカル）」を追加した。新規生成・ブラッシュアップ・部分修正の画面構成、変更・維持指示、プリセット、送信指示文の確認、履歴・PNG保存・次の編集対象を共用する。
...
- 元画像と参照資料は既存の順序で渡す（元画像・マスクを含め合計10枚）。指示文は既存の合成結果をそのまま渡し、追加のプロンプト書換えは行わない。
- 部分修正は既存ブラシから白黒マスクを作り、元画像・参照資料の後に渡す。白が変更・黒が維持。出力は元画像と同寸法とし、既存の局所合成でマスク外を保持する。Qwen部分修正への切替時は局所合成を初期選択する。
...
## 部分修正・参照入力の追加（2026-09-21）

Qwenの部分修正を有効化。白黒マスクはOpenAIの透明マスクとは別形式で、白=編集・黒=維持、最後の画像として渡す。元画像と参照資料の番号は従来の順序を保ち、マスクの役割を最終指示文に追記する。ブラシ・消しゴム・境界ぼかし・局所合成・履歴復元を流用する。

入力合計10枚まで。参照資料のみの上限は新規生成10枚、ブラッシュアップ9枚、部分修正8枚（元画像1枚＋マスク1枚）。画面に現在枚数と上限を表示し、超過アップロードを拒否する。サーバーもモードごとの合計枚数を検証する。Qwen画像の寸法調整ボタンは32の倍数に変更。

実GPU試験: 512×640へ調整した試験画像の本にブラシを塗り、赤色の参照画像を追加。40ステップ・VAE512/384。元画像・参照色・マスクの計3枚で72.73秒、completed。目視で本の表紙が赤くなった。局所合成のRGBA比較では、マスク外の変更画素0、マスク内の変更画素6386/6388。試験ID `9737cc79-464b-4fb5-9680-d182bf37eb9c`、記録は `.tmp/qwen/inpaint-live/latest-result.json`。色参照と文章の寄与を別々に測る比較ではない。

Python110件・JS29件成功。白黒極性、消しゴム、マスクの最後尾配置、透明度を含む範囲外保持、空マスク拒否、寸法不一致拒否、10枚上限と超過拒否を確認。ブラウザーで既存ブラシ→送信指示文→Qwenモック処理→局所合成→下書き復元を確認。コンソールエラーなし。

参照10枚同時の実GPU性能・画質、あらゆるマスク形状・画像での指示追従、2048部分修正の実GPU処理は未検証。範囲外の厳密保持は局所合成後の画像に対するもの。生出力は範囲外が変化する場合がある。


## 元画像クリアと参照付き新規作成

「元画像をクリア」は調整前の下書きを保存して元画像・マスク・移動位置を解除し、参照資料・指示・結果・履歴を保持する。Qwenの参照付き新規生成では、参照はキャンバスではなく人物等の特徴資料であること、背景・構図等は明示指定時のみ引き継ぐことを指示する。顔資料の役割説明も編集用と分離。元画像を入力せず参照資料だけ渡すモックテストと、ブラウザーでクリア後も参照1枚・結果が残ることを確認。修正指示による画質・人物同一性の実GPU比較は未実施。


## ランダムシード指定

「シードを変更」は-1を設定する。-1のジョブは実行開始時に32bit乱数を決め、qwen_seed_usedとして状態変更と一緒に保存する。要求params.qwen_seedは-1のまま保持し、履歴復元・次の編集対象・下書き復元へ引き継ぐ。子プロセスには確定した実行シードを渡す。同一IDの再確認・保存再処理では再抽選しない。履歴に指定値と実行値を分けて表示する。固定シードの動作は従来通り。バックエンド15件・JS30件成功、ブラウザーで生成→結果側と履歴側の次の編集対象で-1を保持することを確認。


## モデルの連続再利用（2026-09-21）

Qwen選択中は、Windows Job Objectで所有する専用子プロセスとパイプラインを保持する。各要求は直列に実行し、完了応答を受けてから画像を回収する。GPT Image・アップスケールへの切替時は解放する。実行中の切替はジョブ完了後に解放する。PE補強・SwinIR実行前にも共通GPUロック内で解放する。Python・モデルパス・オフロード・VAE設定が変わった場合は次回開始時に読み直す。取消、推論失敗、時間超過、サーバー終了でも解放する。起動時には常駐用Job Objectを回収し、古い要求を再実行しない。

CPUオフロード設定は維持するため、モデルの保持場所はRAMとVRAMに分かれる。サーバー全体で1モデルを保持し、画面から最後に通知されたモデル選択を使用する。ブラウザーを閉じただけではアンロードしない。

RTX 5090実測: 512×512・2ステップ・sequential・CFG 1、同一シードで2回のテキスト生成が完了。初回35.30秒、再利用6.75秒。2回目のmodel_reused=trueと終了時のプロセス解放を確認。40ステップ・参照あり・CFG有効時の速度比較、実GPUでの強制終了試験は未実施。モックでは再利用、設定変更、モデル切替API、SwinIR排他、取消、所有プロセス回収を検証。バックエンド127件成功。


## PE-I2I（2026-09-22）

公式仕様: https://huggingface.co/Qwen/Qwen-Image-2.1-PE-I2I

画像なし新規生成はPE-T2I、参照付き新規生成・ブラッシュアップ・部分修正はPE-I2Iを選ぶ。AutoProcessorとAutoModelForImageTextToTextで画像を入力順に渡し、その後に利用者の指示文を置く。元画像は編集時のみ先頭、参照は登録順、部分修正マスクは末尾。通常の指示文に役割説明などを自動追加しない。補強モデル付属のsystem_prompt.txtを使用する。ネガティブは補強文へ混ぜない。

補強前の画像ID・順序・マスクを登録時に固定し、採用時に不一致なら拒否する。wh_ratioまたはratio_followを受け取り表示するが、指定寸法は自動変更しない。採用は利用者の操作のみで、自動画像生成はしない。補強前・補強後・最終送信文を記録する。PE-I2Iモデルのパスはローカルpe-settings.jsonのmodel_i2iに保持する。Qwen常駐モデルは補強開始時に解放し、GPUキューは共有する。

モックのバックエンド130件、JavaScript31件成功。ブラウザーでT2I/I2I表示の切替とコンソールエラーなしを確認。チェックポイント4分割のsafetensorsヘッダーとファイルサイズ整合を確認。

実GPU確認: 参照2枚＋庭園のお茶会の指示でPE-I2I補強成功（146.594秒、生成3244トークン、CUDA最大割当26,330,948,608 bytes）。出力は画像1・画像2の人物を同一テーブルに着席させ、学園の庭園に配置する英語文、推奨比率3:2。画像の自動生成は行っていない。最初の高解像度のままの試験は取消し、公式pe_core.pyに合わせて補強用コピーを1枚あたり1024²画素以下へ縮小した。元ファイル・本生成用画像は変更しない。mm_token_type_idsの互換補完も公式run_transformers.pyに合わせた。前処理追加後のPEテスト13件成功。実GPUでの部分修正マスク補強・10枚入力・補強文採用後の生成品質は未検証。

## モデル保持の解除（2026-09-22）
利用者の要望により常駐を無効化。Qwenは各生成で専用子プロセスを起動し、成功・失敗・取消の後に終了してモデルを解放する。旧常駐プロセスの起動時回収は保持。旧画面の保持APIは常に無効を返す。Qwen回帰テスト18件・UIビルド成功。


## 時間計測オプション（2026-09-22）
Qwen設定の時間計測を有効にすると、子プロセス内の段階別・ステップ別の経過時間をmsで記録し、履歴に表示する。初期値は無効。CUDA同期を挟むため計測自体の負荷あり。初回Transformer呼出し前までを推論準備とし、以後はステップ末コールバック間（初回のみTransformer前から）を計測する。CFGの両推論、CPUオフロード転送などを含み、純GPUカーネル時間ではない。モデル読込、入力準備、画像変換、PNG保存を区別する。キュー待機・子プロセス起動・アプリへの結果登録／書き出しは子プロセス計測外。既存ジョブの時間内訳は後から復元できない。
